    ...
    match ltp_code.get():
        ...
        case "Tutorial":
            ...
            if df_sel_tut.empty:
                ...
                #print(T_used,T_count,deptin_lst_tut,codein_tut,df_curi_tut.Code.tolist())
                code_lst_temp = df_curi_tut.Code.tolist()
                for j in range(len(T_count)):
                    if T_used[j] >= T_count[j]:
                        if df_curi_tut.Code.tolist()[j] in codein_tut:
                            code_lst_temp.remove(df_curi_tut.Code.tolist()[j])
                #print(code_lst_temp)
                if df_sel_tut[df_sel_tut["nick_name"]==faculty_name].empty:
                    ...
                    for i in range(len(my_tut_code)):
                        if my_tut_code[i] in code_lst_temp:
                            if assign_dept_code.get() == df_sel_tut.at[df_sel_tut.index.tolist()[0],"deptT_class"].split(",")[my_tut_code.index(my_tut_code[i])] and semester_code.get() == df_sel_tut.at[df_sel_tut.index.tolist()[0],"semT"].split(",")[my_tut_code.index(my_tut_code[i])]:
                                code_lst_temp.remove(my_tut_code[i])
                    if len(code_lst_temp) == 0:
                        text_box2.delete('1.0','end')
                        text_box2.insert('1.0',f"!! HOUSE-FULL !! for Tutorial slots in {assign_dept_code.get()} department, {semester_code.get()} semester ..........  ")
                    code_lst = code_lst_temp
        case "Practical":
            ...
            if df_sel_lab.empty:
                ...
                if df_sel_lab[df_sel_lab["nick_name"]==faculty_name].empty:
                    ...
                    for i in range(len(my_lab_code)):
                        if my_lab_code[i] in code_lst_temp:
                            if assign_dept_code.get() == df_sel_lab.at[df_sel_lab.index.tolist()[0],"deptP_class"].split(",")[my_lab_code.index(my_lab_code[i])] and semester_code.get() == df_sel_lab.at[df_sel_lab.index.tolist()[0],"semP"].split(",")[my_lab_code.index(my_lab_code[i])]:
                                code_lst_temp.remove(my_lab_code[i])
                    if len(code_lst_temp) == 0:
                        text_box2.delete('1.0','end')
                        text_box2.insert('1.0',f"!! HOUSE-FULL !! for Practical slots in {assign_dept_code.get()} department, {semester_code.get()} semester ..........  ")
                    code_lst = code_lst_temp           
        case "Projects":
...
text_box1=Text(lf2, width=60,height=3,fg="Blue",font=("Helvetica", 11, "bold"))
...
text_box2=Text(lf4, width=60,height=3,fg="Blue",font=("Helvetica", 11, "bold"))
